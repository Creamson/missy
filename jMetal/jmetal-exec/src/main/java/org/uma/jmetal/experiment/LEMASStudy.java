    ...
    static List<ExperimentAlgorithm<DoubleSolution, List<DoubleSolution>>> configureAlgorithmList(
        ...
        for (int i = 0; i < problemList.size(); i++) {
            Algorithm<List<DoubleSolution>> algorithm = new JMetal5BaseEMAS<>(problemList.get(i).getProblem(),
                    ...
                    "BaseEMAS", 0, new EmasDominanceComparator(), new EmasDominanceComparator());
        ...
        for (int i = 0; i < problemList.size(); i++) {
            Algorithm<List<DoubleSolution>> algorithm = new JMetal5BaseEMAS<>(problemList.get(i).getProblem(),
                    ...
                    Constants.MAX_ITERATIONS, Constants.NUMBER_OF_ISLANDS, Constants.ENV_ENERGY, Constants.INITIAL_RESOURCE_VALUE,
                    Constants.TRANSFER_RESOURCE_VALUE,
                    "AreaUnderControl", 0, new AreaUnderControlComparator(), new EmasDominanceComparator());
            algorithms.add(new ExperimentAlgorithm<>(algorithm, algorithm.getName(), problemList.get(i).getTag()));
        }
        ...
        for (int i = 0; i < problemList.size(); i++) {
            Algorithm<List<DoubleSolution>> algorithm = new JMetal5BaseEMAS<>(problemList.get(i).getProblem(),
                    ...
                    Constants.MAX_ITERATIONS, Constants.NUMBER_OF_ISLANDS, Constants.ENV_ENERGY, Constants.INITIAL_RESOURCE_VALUE,
                    Constants.TRANSFER_RESOURCE_VALUE,
                    "AlphaDominance", 0, new AlphaDominanceComparator(Constants.ALPHA_VALUES), new EmasDominanceComparator());
            algorithms.add(new ExperimentAlgorithm<>(algorithm, algorithm.getName(), problemList.get(i).getTag()));
        }
